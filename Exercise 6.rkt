...
(define-struct (safe chest)
  ...
  (define (open s)
    (if (safe-locked? s)
        ...
        (set-chest-open?! s true)
        )
    )
  
  (define (unlock s combo)
    (if (= combo (safe-combination s))
        (set-safe-locked?! s false)
        (printf "wrong combination")
        )
    )
  )

;; Add your define-struct for box here.
;; It should be a subtype of thing, and it should have no additional
;; fields or methods.



;; new-box string container -> box
;; Makes a new box with the specified adjectives, in the specified location,
;; and initializes it.
...
;; Add your define-struct for chest here.
;; It should be a subtype of box, and it should have a field named open?



;; new-chest: string container -> chest
;; Makes a new chest with the specified adjectives, in the specified location,
;; and initializes it.
(define (new-chest adjectives location)
  (local [(define the-chest (make-chest (string->words adjectives)
                                        '() location
                                        false))]
...
(define-user-command (open chest)
  "Opens the chest so you can see what's inside of it")
(define-user-command (close chest)
  "Closes the chest")


;; Add your define-struct for safe here.
;; It should be a subtype of chest, and it should have fields named
;; locked? and combination



;; new-safe: string container -> safe
;; Makes a new safe with the specified adjectives and combination,
;; in the specified location, and initializes it.
(define (new-safe adjectives combination location)
...
(define-user-command (unlock safe combination)
  "Try to unlock the safe using the specified combination")
...
(define (start-game)
  ...
  (local [(define starting-room (new-room ""))
          (define (new-coin location)
            (new-prop "coin" "It's a dingey old coin" location))]
    (begin (set! me (new-person "" starting-room))
           (new-coin (new-box "red" starting-room))
           (new-coin (new-chest "green" starting-room))
           (new-coin (new-safe "blue" 12345 starting-room))
           (check-containers!)
           (void))))
