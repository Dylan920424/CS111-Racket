...
(define (related? a1 a2)
  (if (empty? a1)
      false
      (if (member (human-name a1) (ancestors-names-inclusive a2))
          true
          (or (related? (human-parent-1 a1) a2)
              (related? (human-parent-2 a1) a2)
              )
          )
      )
  )
...
(define (list-contents tree)
  (if (empty? tree)
      '()
      (append (list-contents (node-smaller tree))
...
(define (lookup social-num pn)
  (if (empty? pn)
      "not found"
      (if (= social-num (person-ssn (node-person pn)))
