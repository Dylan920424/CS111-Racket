...
#reader(lib "htdp-advanced-reader.ss" "lang")((modname adventure-DESKTOP-7C9LLNJ) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #t #t none #f () #f)))
...
(define-struct (cup prop)
  ...
  #:methods
  (define (fill cup)
    (set-cup-full?! cup true)
    )
  
  (define (drink-from cup)
    (if (cup-full? cup)
        (begin (set-cup-full?! cup false)
               (printf "you drank from the cup\n")
               )
        (printf "the cup is empty\n")
        )
    )
  )

(define (new-cup description examine-text location full?)
  (local [(define words (string->words description))
          (define noun (last words))
          (define adjectives (drop-right words 1))
          (define cup (make-cup adjectives '() location noun examine-text full?))]
    (begin (initialize-thing! cup)
           cup)))

(define-struct (water-fountain prop)
  ()
  #:methods
  (define (fill-cup water-fountain cup)
    (if (have? (the cup))
        (if (cup-full? cup)
            (printf "your cup is full already\n")
            (begin (printf "you have filled your cup with water\n")
                   (set-cup-full?! cup true)
                   )
            )
        (printf "you do not have a cup to fill with\n")
        )
    )
  )

(define (new-water-fountain description examine-text location)
  (local [(define words (string->words description))
          (define noun (last words))
          (define adjectives (drop-right words 1))
          (define water-fountain (make-water-fountain adjectives '() location noun examine-text))]
    (begin (initialize-thing! water-fountain)
           water-fountain)))

(define-struct (notebook prop)
  (book-content)

  #:methods
  (define (write-in notebook content)
    (if (have? (the pen))
        (set-notebook-book-content! notebook (string-append (notebook-book-content notebook) (string-append content "\n")))
        (printf "you don't have a pen to write with!\n")
        )
    )

  (define (read notebook)
    (if (string=? (notebook-book-content notebook) "")
        (printf "the book is empty\n")
        (display-line (string-append "notebook: \n" (notebook-book-content notebook))
                      )
        )
    )
  )

(define (new-notebook description examine-text location content)
  (local [(define words (string->words description))
          (define noun (last words))
          (define adjectives (drop-right words 1))
          (define notebook (make-notebook adjectives '() location noun examine-text ""))]
    (begin (initialize-thing! notebook)
           notebook)))
...
(define-user-command (drink-from cup)
  "Drinks the liquid in the cup if full"
  )

(define-user-command (fill-cup water-fountain cup)
  "Fills the cup up with water from the water fountain"
  )

(define-user-command (write-in notebook msg)
  "Writes the message inside the notebook"
  )

(define-user-command (read notebook)
  "Displays the information written in the notebook"
  )
...
(define (start-game)
  ...
  (local [(define starting-room (new-room ""))]
    (begin (set! me (new-person "" starting-room))
           ...
           (new-cup "cup full of water" "there is water in the cup" starting-room true)
           (new-notebook "old notebook" "this a notebook" starting-room "")
           (new-prop "black pen" "there's ink in this pen" starting-room)
           (new-water-fountain "nasty water fountain" "get water from this fountain" starting-room)
           (check-containers!)
           (void))))
...
(define (walkthrough)
  (begin (drink-from (the cup))
         (drink-from (the cup))
         (fill-cup (the water fountain) (the cup))
         (take (the cup))
         (fill-cup (the water fountain) (the cup))
         (fill-cup (the water fountain) (the cup))
         (drink-from (the cup))
         (fill-cup (the water fountain) (the cup))
         (read (the notebook))
         (write-in (the notebook) "dear diary")
         (take (the pen))
         (write-in (the notebook) "dear diary")
         (read (the notebook))
         (write-in (the notebook) "this is a bad game")
         (read (the notebook))
         )
  )
